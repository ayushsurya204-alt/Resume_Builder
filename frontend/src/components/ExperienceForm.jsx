...
const ExperienceForm = ({data,onChange}) => {
  ...
  const generateDescription = async (index) => {
    ...
    const propmt = `enhance this job description ${experience.description} for the position of ${experience.position} 
    at ${experience.company}`
    try {
      const {data} = await api.post('/api/ai/enhance-job-desc',{userContent:propmt},{headers:{Authorization:token}})
            ...
            {data.map((experience,index)=>(
             ...
             <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                ...
                disabled:opacity-50'>
                  {generatingIndex === index ? (
                    <Loader2 className='w-3 h-3 animate-spin'/>) : (  <Sparkles className='w-3 h-3'/>)
                  } 
                  Enhance with AI
                </button>
              </div>
              ...
              {(e)=> updateExperience(index,"description",e.target.value)}
               className='w-full text-sm px-3 py-2 rouned-lg resize-none'
